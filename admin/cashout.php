...
if(isset($_GET['id']) && isset($_GET['cashout'])){
    ...
    if($cashout=="true"){
        ...
        while($row=mysqli_fetch_assoc($res)){
            ...
            // Check if the user exists in the 'point' table
            $check = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM `point` WHERE `uid`='$uid'"));
            $point=$check['amount'];
            if ($point < 0) {
                // User exists, fetch the original amount
                ...
                mysqli_query($con, "UPDATE `cashout` SET `comfirm`='true' WHERE `id`='$id'");
                    ...
                    ?>
                    <script>
                        window.alert('အသုံးပြုသူတွင်ငွေကြေးလုံလောက်မှုမရှိပါ');
...
?>
  ...
  <div class="banner">
        <div class="header-main">
            ...
            <div class="result_container" id="cashout" >
                ...
                <table>
                    ...
                    <?php
                        $res=mysqli_query($con,"SELECT * FROM `cashout`  ORDER BY `id` DESC");
                        while($row=mysqli_fetch_assoc($res)){
                            $uid=$row['uid'];
                    ...
                    <tr style="border-bottom:1px dashed #19E9F0">
                        <td style="border-right:1px dashed #19E9F0"><?php echo $row['uname'] ?></td>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        document.getElementById('cashout').addEventListener('click', function(event) {
                            // Check if the clicked element has the class 'buy-btn'
                            if (event.target.classList.contains('buy-btn')) {
                                // Display the confirmation box
                                document.getElementById('confirm-box').style.display = 'block';
                                // Set the data-id attribute of confirm-yes button
                                document.getElementById('confirm-yes').setAttribute('data-id', event.target.getAttribute('data-id'));
                            }
                        });

                        // Add event listener for the 'confirm-yes' button
                        document.getElementById('confirm-yes').addEventListener('click', function() {
                            // Hide the confirmation box
                            document.getElementById('confirm-box').style.display = 'none';
                            // Get the data-id attribute value
                            var id = this.getAttribute('data-id');
                            // Redirect to cashout.php with the id and parameter
                            window.location.href = 'cashout.php?id=' + id + '&cashout=true';
                        });

                        // Add event listener for the 'confirm-no' button
